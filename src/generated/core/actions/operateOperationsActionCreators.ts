...
const api = new gapi.OperateOperationsApi(
  conf,
...
export const doOperateNamespaceKey = ({
  ...
  dispatch: ThunkDispatch<
    StoreState,
    void,
    OperateNamespaceKeyAction | RouterAction
  >
) => {
...
export const doOperateNamespaceSetKey = ({
  ...
  dispatch: ThunkDispatch<
    StoreState,
    void,
    OperateNamespaceSetKeyAction | RouterAction
  >
) => {
  const calls = new ApiActionCreatorExtender<Record, RestClientError>(
    {
  ...
  try {
    ...
    const xformedError = await xformApiResponse(error);
    return calls.error(
      [operateNamespaceSetKeyFailed(xformedError)],
      xformedError
    );
  }
};
